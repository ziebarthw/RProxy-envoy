...
struct _RpAcceptedSocketImpl {
    ...
    RpNetworkAddressInstance* m_local_address;
    RpNetworkAddressInstance* m_remote_address;
...
create_remote_address(int sockfd)
{
...
static inline RpNetworkAddressInstance*
create_local_address(struct sockaddr* addr)
{
...
rp_accepted_socket_impl_new(RpIoHandle* io_handle, UNIQUE_PTR(evhtp_connection_t) conn)
{
    ...
    RpNetworkAddressInstance* remote_address = create_remote_address(conn->sock);
    RpNetworkAddressInstance* local_address = create_local_address(conn->saddr);
