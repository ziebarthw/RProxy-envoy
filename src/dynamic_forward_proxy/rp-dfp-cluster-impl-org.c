...
struct _RpDfpClusterImpl {
    RpStaticClusterImpl parent_instance;
...
create_server(RpDfpCluster* self, const char* host_name, int port)
{
    ...
    rule_t* rule = rule_new(rule_cfg);

    rproxy_t* rproxy = rp_server_factory_context_bootstrap(me->m_server_context);
    {
        G_MUTEX_AUTO_LOCK(rp_server_factory_context_lock(me->m_server_context), locker);
...
create_sub_cluster_config_i(RpDfpCluster* self, const char* cluster_name, const char* host_name, int port)
{
    ...
    rule_t config = me->m_orig_cluster_config;
    rule_cfg_t* rule_cfg = config.config;

//    rp_cluster_cfg_set_name(&config, cluster_name);
//    rp_cluster_cfg_set_type(&config, RpDiscoveryType_STRICT_DNS);
    rule_cfg->discovery_type = RpDiscoveryType_STRICT_DNS;
...
RpDfpClusterImpl*
rp_dfp_cluster_impl_new(const rule_t* config, RpClusterFactoryContext* context, RpStatusCode_e* creation_status)
//RpDfpClusterImpl*
//rp_dfp_cluster_impl_new(const RpClusterCfg* config, RpClusterFactoryContext* context, RpStatusCode_e* creation_status)
{
    ...
    self->m_server_context = rp_cluster_factory_context_server_factory_context(context);
...
//RpClusterImplBase*
//rp_dfp_cluster_impl_create_cluster_with_config(const RpClusterCfg* cluster, RpClusterFactoryContext* context)
RpClusterImplBase*
rp_dfp_cluster_impl_create_cluster_with_config(const rule_t* cluster, RpClusterFactoryContext* context)
{
